...
% This script is used to reproduce the results reported in STRON (Chauhan et al 2018).
...
cg_iters = 25;
%% Method: TRON, STRON, STRON_SVRG, STRON_PCG, STRON_PCG_PROG, SQN, SVRG_LBFGS, SVRG_SQN, LBFGS, NEWTON_CG
% problems: SVM_L2R_L2L LOGISTIC_REGRESSION_L2R
rand('seed', 2018); randn('seed', 2018);
% options.batch_size = 10;
% options.batch_hess_size = options.batch_size * 20; 
w{1} = zeros(p,1);
[info{1}.cost, info{1}.time, info{1}.accuracy, info{1}.grad_calc_count] = interface(w{1}, x_train, x_test, y_train,...
      y_test, lambda, batch_size, max_iters, 'STRON_SVRG', 'LOGISTIC_REGRESSION_L2R', cg_iters);
...
%% Display Results
% 3
fig1 = plot_graph(opt_obj, 'time','optimality_gap', {'STRON-SVRG', 'STRON', 'TRON'}, info, [50 100 600 500]);
% saveas(fig1,'result1.eps', 'epsc');
fig2 = plot_graph(opt_obj, 'time','accuracy', {'STRON-SVRG', 'STRON', 'TRON'}, info, [700 100 600 500]);
